...
fileName = open('Overall_Model.pickle', 'rb')
Overall = pickle.load(fileName)

fileName = open('Recent_Model.pickle', 'rb')
Recent = pickle.load(fileName)

fileName = open('Home_Model.pickle', 'rb')
Home = pickle.load(fileName)

fileName = open('Away_Model.pickle', 'rb')
Away = pickle.load(fileName)
...
@app.route('/api/Rec-1', methods=['GET'])
def fetchImages():
...
@app.route('/api/Rec-1/slectedPlayer', methods=['POST'])
def playerSelection():
...
@app.route('/api/Rec-2', methods=['GET'])
...
@app.route('/api/Rec-2/selectedPlayers', methods=['POST'])
def selectedPlayers():
...
@app.route('/api/Rec-3', methods=['GET'])
def RankImages_rec3():
...
def RecentPredict(row):
    ...
    df = pd.DataFrame(data)
    test = df[['form_innings', 'form_runs', 'recent_not_outs', 'recent_100s',
               'recent_50s', 'recent_30s', 'recent_zeros', 'form_average']]
