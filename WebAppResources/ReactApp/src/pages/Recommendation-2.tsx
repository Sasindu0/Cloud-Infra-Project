...
function Rec_2() {
  ...
  const fetchImages = async () => {
    const { data } = await axios.get(`${baseUrl}/api/Rec-2`);
    setImages(data);
  ...
  const selectedPlayers = async () => {
    ...
    const { data } = await axios.post(`${baseUrl}/api/Rec-2/selectedPlayers`, {
      ids: selectedIds,
  ...
  function pageRedirect() {
    window.location.href = "/recommendation/3";
  }
